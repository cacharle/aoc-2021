    ...
    where T: Fn(usize, usize) -> u32
{
    ...
    while nums_copy.len() > 1 {
        ...
        nums_copy = nums_copy.iter().filter(|bs| bs[i] == target).cloned().collect();
        i += 1;
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let nums: Vec<Vec<u32>> = file_content
        ...
        .collect::<Option<_>>().ok_or("cannot parse bit")?;
    ...
    println!("part2 result: {}", c02_scrubber_rating * oxygen_generator_rating);
