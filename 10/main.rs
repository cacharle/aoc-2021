...
impl Delimiter {
    fn from_char(c: char) -> Delimiter {
        use DelimiterKind::*;
        match c {
            '(' => Delimiter { open: true, kind: Parent },
            '[' => Delimiter { open: true, kind: Square },
            '{' => Delimiter { open: true, kind: Curly },
            '<' => Delimiter { open: true, kind: Arrow },
            ')' => Delimiter { open: false, kind: Parent },
            ']' => Delimiter { open: false, kind: Square },
            '}' => Delimiter { open: false, kind: Curly },
            '>' => Delimiter { open: false, kind: Arrow },
            _ => panic!("not a valid delimiter {}", c),
        }
    }

    fn pair(&self) -> Delimiter {
        Delimiter { open: !self.open, kind: self.kind }
    }
...
impl FromStr for Delimiters {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(Delimiters(s.chars().map(|c| Delimiter::from_char(c)).collect()))
    }
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let lines: Vec<Delimiters> = file_content.lines().map(|l| l.parse()).collect::<Result<_, _>>()?;

    let mut counter = HashMap::new();
    let mut autocomplete_scores = Vec::new();
    for delimiters in &lines {
        match delimiters.parse() {
            DelimitersResult::Corrupted(delimiter) =>
                *counter.entry(delimiter).or_insert(0) += 1,
    ...
    println!("part2 result: {}", autocomplete_scores[autocomplete_scores.len() / 2]);
